...
class OllamaTranslator(TranslationEngine):
    ...
    def _load_glossary(self, glossary_path: str):
        ...
        try:
            ...
            logging.info(
                f"載入術語表：{len(self.preserve_terms)} 個保留術語，{len(self.translation_dict)} 個翻譯術語"
            )
        except Exception as e:
    ...
    def translate(self, text: str, source_lang: str, target_lang: str) -> str:
        ...
        is_chinese_conversion = (
            source_lang in ["zh", "zh-cn"] and target_lang == "zh-tw"
        ) or (source_lang in ["zh", "zh-tw"] and target_lang == "zh-cn")
