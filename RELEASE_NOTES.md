...
## v3.5.0 - 日誌標準化與系統優化 (2026-01-02) 📋

### 重大里程碑

此版本完成了系統級的日誌標準化 (Logging Standardization) 專案，消除了所有的技術債，將分散的 `print()` 語句轉換為統一的結構化日誌系統。

### ✨ 核心改進

#### 1. 統一的日誌系統
- **結構化輸出**: 所有模組 (Core, Processors, CLI) 現在使用統一的日誌格式。
- **可配置級別**: 支援 `LOG_LEVEL` (DEBUG, INFO, WARNING, ERROR) 環境變數控制。
- **檔案輸出**: 自動將日誌旋轉並儲存至 `logs/` 目錄。
- **上下文資訊**: 日誌包含時間戳、模組名稱與行號，大幅提升除錯效率。

#### 2. 技術債清除
- **360+ 個 Print 移除**: 移除了程式碼庫中遺留的除錯用 `print()` 語句。
- **異常處理強化**: 所有的 `try-except` 區塊現在都會記錄完整的堆疊追蹤。
- **配置集中化**: 建立了 `Settings` 類別，統一管理應用程式配置。

#### 3. 前端架構統一
- **Single Source of Truth**: 移除了過時的 `web/` 目錄，統一使用 `web-frontend/`。
- **Static Export**: 配置 Next.js 進行靜態導出，優化 Vercel 部署流程。
- **API 整合**: 完善了與 FastAPI 後端的整合路徑。
