...
class BenchmarkRunner:
    ...
    def run_benchmark(self, pdf_path: str, mode: str = "basic", dpi: int = 150) -> Dict:
        """執行基準測試"""
        ...
        all_results, _ = ocr_tool.process_pdf(pdf_path, dpi=dpi, show_progress=False)
    ...
    def _print_plain_table(self):
        ...
        for r in self.results:
            test_name = f"{r['mode']}/{r['dpi']}"
            print(
                f"{test_name:<20} {r['pages']:<8} {r['total_time']:.2f}s      "
...
def main():
    ...
    try:
        ...
        # 總結
        print("\n📊 測試總結:")
        avg_time_per_page = sum(r["time_per_page"] for r in runner.results) / len(
            runner.results
        )
        ...
        print(
            f"   最快測試: {min(runner.results, key=lambda x: x['time_per_page'])['mode']}/{min(runner.results, key=lambda x: x['time_per_page'])['dpi']}"
        )
        print(
            f"   最省記憶體: {min(runner.results, key=lambda x: x['memory_used'])['mode']}/{min(runner.results, key=lambda x: x['memory_used'])['dpi']}"
        )
