...
export default function Home() {
  ...
  const [claudeKey, setClaudeKey] = useState<string | undefined>(undefined);
  const [ocrMode, setOcrMode] = useState<string>('hybrid');

  useEffect(() => {
    setGeminiKey(localStorage.getItem('gemini_api_key') || undefined);
    setClaudeKey(localStorage.getItem('claude_api_key') || undefined);
  ...
  const processFile = (file: File) => {
    ...
    uploadFile(file, ocrMode, useGemini ? geminiKey : undefined, useClaude ? claudeKey : undefined);
  };
    ...
    <>
      ...
      {result && (
        <EditorModal
          ...
          initialText={result.results?.raw_result || ''}
        // fileUrl={`/api/files/${result.file_name}/download`} // Assume such prop or data exists, placeholder for now
        />
      )}
    </>
  );
