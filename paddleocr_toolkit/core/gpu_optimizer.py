...
class GPUBatchProcessor:
    ...
    def _gpu_predict_batch(self, batch_tensor: np.ndarray, ocr_engine: Any) -> List[Any]:
        """
        GPU批次预测
    ...
    def get_performance_stats(self) -> dict:
        ...
        return {
            **self.stats,
            ...
            'avg_time_per_batch': self.stats['total_time'] / self.stats['total_batches'] if self.stats['total_batches'] > 0 else 0,
            'preprocessing_ratio': self.stats['preprocessing_time'] / self.stats['total_time'] if self.stats['total_time'] > 0 else 0,
            'gpu_ratio': self.stats['gpu_time'] / self.stats['total_time'] if self.stats['total_time'] > 0 else 0,
        }
    ...
    def print_performance_report(self):
        ...
        print(f"预处理时间: {stats['preprocessing_time']:.2f}s ({stats.get('preprocessing_ratio', 0):.1%})")
