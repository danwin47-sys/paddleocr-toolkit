...
def config_wizard():
    ...
    config['ocr']['device'] = prompt(
        "选择计算设备",
        default='gpu',
        options=['gpu', 'cpu']
    )
    
    config['ocr']['dpi'] = int(prompt(
        "PDF转换DPI (建议: 150-300)",
        default='200'
    ))
    ...
    config['ocr']['lang'] = prompt(
        "识别语言",
        default='ch',
        options=['ch', 'en', 'korean', 'japan', 'chinese_cht']
    )
    ...
    config['output']['directory'] = prompt(
        "输出目录",
        default='./output'
    )
    ...
    config['performance'] = {}
    config['performance']['batch_size'] = int(prompt(
        "批次大小 (建议: 4-16)",
        default='8'
    ))
    
    config['performance']['max_workers'] = int(prompt(
        "最大工作线程 (建议: 2-8)",
        default='4'
    ))
    ...
    config['logging']['file'] = prompt(
        "日志文件路径",
        default='./logs/paddleocr.log'
    )
