...
from paddleocr_toolkit.llm import LLMClient, create_llm_client
...
class SemanticProcessor:
    ...
    def __init__(
        self, llm_provider: str = "ollama", model: Optional[str] = None, **llm_kwargs
    ):
        """
        初始化語義處理器
        ...
        try:
            self.llm_client = create_llm_client(
                provider=llm_provider, model=model, **llm_kwargs
            )
    ...
    def correct_ocr_errors(
        self, text: str, context: str = "", language: str = "zh"
    ) -> str:
        """
        使用 LLM 修正 OCR 錯誤
        ...
        try:
            corrected_text = self.llm_client.generate(
                prompt, temperature=0.3, max_tokens=len(text) * 2  # 較低溫度保持準確性  # 預留足夠空間
            )
...
"""
    ...
    def extract_structured_data(
        self, text: str, schema: Dict[str, Any], language: str = "zh"
    ) -> Dict[str, Any]:
        """
        根據 Schema 提取結構化資料
