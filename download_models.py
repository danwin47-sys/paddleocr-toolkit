...
if paddleocr_dir.exists():
    initial_size = sum(f.stat().st_size for f in paddleocr_dir.rglob('*') if f.is_file()) / (1024 * 1024)
...
# 檢查下載後的大小
if paddleocr_dir.exists():
    final_size = sum(f.stat().st_size for f in paddleocr_dir.rglob('*') if f.is_file()) / (1024 * 1024)
